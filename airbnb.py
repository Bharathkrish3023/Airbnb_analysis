...
def mainfile():
    file=pd.read_csv("C:/Capstone files/Airbnb_main.csv")
    return file
...
def geoGraph():
  ...
  fig=px.scatter_mapbox(air_df1, lon='Longitude', lat='Latitude', color='Price', size='Accommodates', color_continuous_scale='rainbow', hover_name='SubUrb', range_color=(0, 200), mapbox_style='carto-positron', zoom=0.4)
...
def country_filter(country):
  ...
  #Plot
  fig=px.scatter_mapbox(country_df, lon='Longitude', lat='Latitude', color='Price', size='Accommodates', color_continuous_scale='rainbow', hover_name='Name', range_color=(0, 200), mapbox_style='carto-positron', zoom=11)
...
def price_analysis(df):
  ...
  with col1:  
    fig=px.bar(clear_price, x='Room Type', y='Price', hover_name='Name', title=f'Price Analysis and Commonly Preferred Room Type in {country}', color_discrete_sequence=px.colors.sequential.Aggrnyl_r)
    st.plotly_chart(fig)
